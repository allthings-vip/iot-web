...
class EditModal extends Component {
    ...
    handleChange = (item) => 
    {
        if(item.fileList.length>0){
            if(item.fileList[0].response){
                this.setState({ imageUrl:item.fileList[0].response.ret || '' })
            }
        } else {
                ...
                <Modal
                    ...
                    <FormItem {...formLayout} label="项目名称">
                                {getFieldDecorator('projectName', {
                                    ...
                                    rules: [{required: true, message: '项目名称不能为空'}, {max: 32, message: '最多输入32个字符'}]
                        ...
                        <FormItem {...formLayout} label="协作人员">
                            {getFieldDecorator('iotUserIds', {
                                ...
                                <Select size="default" placeholder="请选择协作人员"  mode="multiple" onDeselect={this.onDeselect}>
                                    {(collaborators || []).map(v=>{
